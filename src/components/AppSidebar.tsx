...
import { exportSession as buildSessionExport, buildExportFilename, type ExportFormat } from "@/lib/session/session-export";
...
interface AppSidebarProjectActions {
  ...
  onOpenRelay: (projectId: string) => void;
  ...
  const exportSession = useCallback(
    async (sessionId: string, format: ExportFormat) => {
      ...
      try {
        ...
        if (!persisted) {
          toast.error("Could not load session for export");
          return;
        }
        const project = projects.find((p) => p.id === session.projectId);
        const content = buildSessionExport(persisted, format, project);
        const defaultFileName = buildExportFilename(persisted, format);
        const result = await window.claude.sessions.export({ format, content, defaultFileName });
        if (result.canceled) return;
        if (result.error) {
          toast.error("Export failed", { description: result.error });
          return;
        }
        toast.success("Session exported", { description: result.filePath });
      } catch (err) {
        const message = reportError("SIDEBAR:EXPORT_SESSION", err, { sessionId, format });
                ...
                {filteredProjects.map((project) => {
                  ...
                  return (
                    <ProjectSection
                      ...
                      onOpenRelay={() => onOpenRelay(project.id)}
