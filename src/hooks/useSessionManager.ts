...
export function useSessionManager(
  ...
  const opencodeSessionId = (activeEngine === "opencode" && activeSessionId !== DRAFT_ID) ? activeSessionId : null;
  ...
  const codexPlanModeEnabled = activeEngine === "codex"
    ...
    : false;
  const opencodeSessionModel = (activeEngine === "opencode" && activeSessionId !== DRAFT_ID)
    ? (sessions.find((s) => s.id === activeSessionId)?.model ?? startOptions.model)
    : undefined;
  ...
  const { claude, acp, codex, opencode, engine } = primaryPane;
  const { messages, totalCost, contextUsage } = primaryPane;
  const { modelUsage } = primaryPane;
  ...
  const modelUsageRef = useRef(modelUsage);
  modelUsageRef.current = modelUsage;
  ...
  const { reviveSession, reviveAcpSession, reviveCodexSession, reviveOpenCodeSession } = useSessionRevival({
    refs,
    setters,
    engines,
    findProject,
  ...
  const loadSplitPaneBootstrap = useCallback(async (sessionId: string): Promise<SessionPaneBootstrap | null> => {
    ...
    if (backgroundState) {
      return {
        ...
        initialMeta: {
          ...
          modelUsage: backgroundState.modelUsage,
    ...
    return {
      ...
      initialMeta: {
        ...
        modelUsage: persistedSession.modelUsage,
      },
  ...
  return {
    ...
    interrupt: async () => {
      ...
      if (activeSessionIdRef.current === DRAFT_ID
          && startOptionsRef.current.engine === "opencode"
          && isProcessingRef.current) {
        await window.claude.opencode.abortPendingStart();
    ...
    refreshMcpStatus: isOpenCode || isACP || isCodex
      ? (() => Promise.resolve())
    ...
    reconnectMcpServer: isOpenCode
      ? async (_name: string) => { /* OpenCode manages providers and tools itself. */ }
      : isACP
      ? isDraft
        ? async (_name: string) => {
    ...
    supportedModels: isCodex
      ...
      : isACP
        ? []
        : isOpenCode
          ? opencode.models.map<ModelInfo>((model) => ({
              value: model.id,
    ...
    restartWithMcpServers: isOpenCode
      ? async (_servers: McpServerConfig[]) => { /* OpenCode does not consume Harnss MCP config. */ }
      : isACP
      ? isDraft
