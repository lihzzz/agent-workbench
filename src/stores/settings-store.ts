...
interface GlobalSettingsState {
  ...
  /** Monthly spend budget in USD for the usage dashboard. null = no budget set. */
...
interface SettingsActions {
  ...
  setMonthlyBudgetUsd: (budget: number | null) => void;
...
function migrateFromLegacyLocalStorage(): { global: GlobalSettingsState; projects: Record<string, ProjectSettings> } {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    const perProjectSuffixes = [
      "-model-claude", "-model-acp", "-model-codex", "-model-opencode", "-model",
...
function readLegacyProjectSettings(pid: string): ProjectSettings {
  return {
    modelsByEngine: {
      ...
      opencode: readLegacyModelForEngine(pid, "opencode"),
    },
...
export const useSettingsStore = create<SettingsStore>()(
  persist(
    (set, get) => ({
      ...
      setMonthlyBudgetUsd: (budget) =>
        set({ monthlyBudgetUsd: budget !== null && budget > 0 ? budget : null }),
