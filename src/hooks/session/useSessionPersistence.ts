...
import type { PersistedSession, ClaudeEvent, SystemInitEvent, EngineId, ACPSessionEvent, ACPPermissionEvent, ACPTurnCompleteEvent, OpenCodeSessionEvent } from "@/types";
...
export function useSessionPersistence({
  ...
  // Preserve native engine IDs when a delayed save races with session startup.
  const persistSessionWithCodexFallback = useCallback(async (data: PersistedSession) => {
    ...
    if (data.engine === "opencode" && !data.opencodeSessionId) {
      try {
        const existing = await window.claude.sessions.load(data.projectId, data.id);
        if (existing?.opencodeSessionId) payload = { ...payload, opencodeSessionId: existing.opencodeSessionId };
      } catch {
        // Best-effort fallback only.
      }
    }
  ...
  useEffect(() => {
    const handleSessionExit = (sid: string) => {
      ...
      if (sid !== activeSessionIdRef.current && backgroundStoreRef.current.has(sid)) {
        ...
        // Skip persisting an empty/partial background snapshot — a lazily
        // created background store entry (getOrCreate) may hold [] before the
        // session was ever seeded with its full history. Persisting that would
        // clobber the on-disk transcript. The IPC layer guards this too, but
        // skipping here avoids a needless write round-trip.
        if (bgState && session && bgState.messages.length > 0) {
    ...
    const unsubOpenCodeExit = window.claude.opencode.onExit((data) => handleSessionExit(data._sessionId));
    return () => {
      unsubExit();
      unsubAcpExit();
      unsubCodexExit();
  ...
  useEffect(() => {
    ...
    const unsubOpenCode = window.claude.opencode.onEvent((event: OpenCodeSessionEvent) => {
      const sid = event._sessionId;
      if (!sid || sid === activeSessionIdRef.current || visibleSplitSessionIdsRef.current.includes(sid)) return;
      backgroundStoreRef.current.handleOpenCodeEvent(event);
    });

    const unsubOpenCodePermission = window.claude.opencode.onPermissionRequest((data) => {
      const sid = data._sessionId;
      if (!sid || sid === activeSessionIdRef.current || visibleSplitSessionIdsRef.current.includes(sid)) return;
      backgroundStoreRef.current.setPermission(sid, {
        requestId: data.requestId,
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      // Guard against races: between scheduling and firing (2s), a session
      // switch/revival can reset the engine's messages to [] (useEngineBase
      // reset effect). Re-check at fire time that (a) we're still on the same
      // session the timer was scheduled for, and (b) messages are non-empty —
      // otherwise we'd persist an empty array over intact history.
      ...
      const data: PersistedSession = {
        ...
        ...(Object.keys(modelUsageRef.current).length > 0 ? { modelUsage: modelUsageRef.current } : {}),
    ...
    async (sessionId: string, message: string, projectPath: string, titleEngine?: EngineId) => {
      ...
      try {
        ...
        const result = await window.claude.generateTitle(
          ...
          projectPath,
          titleEngine,
          titleEngine === "acp" || titleEngine === "opencode" ? sessionId : undefined,
        );
