...
export function useSessionCache({
      ...
      setSessions((prev) =>
        prev.filter((s) => s.id !== DRAFT_ID).map((s) => ({
          ...
          ...(s.id === id ? {
            ...
            ...(data.workflowGroupId ? { workflowGroupId: data.workflowGroupId } : {}),
            ...(data.workflowGroupName ? { workflowGroupName: data.workflowGroupName } : {}),
            ...(data.stageRole ? { stageRole: data.stageRole } : {}),
            ...(typeof data.stageIndex === "number" ? { stageIndex: data.stageIndex } : {}),
            ...(data.handoffFromSessionId ? { handoffFromSessionId: data.handoffFromSessionId } : {}),
  ...
  useEffect(() => {
    if (activeEngine !== "opencode") return;
    if (engines.opencode.models.length > 0) return;
    const preferredModel = activeSessionId === DRAFT_ID
      ? startOptionsRef.current.model
      : sessionsRef.current.find((session) => session.id === activeSessionId)?.model;
    void prefetchOpenCodeModels(preferredModel);
  }, [
    activeEngine,
    activeSessionId,
    engines.opencode.models.length,
    prefetchOpenCodeModels,
    projects,
    startOptionsRef,
  ]);
