...
import { MAX_FILE_READ_BYTES, readTextFile, writeTextFile } from "../lib/file-io";
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle("shell:show-item-in-folder", async (_event, filePath: string) => {
    try {
      // Expand a leading ~ to the home directory so callers can pass paths like
      // "~/.claude/skills". If the target doesn't exist yet, fall back to opening
          ...
          for (const file of matchingFiles) {
            ...
            try {
              ...
              if (!fileStat.isDirectory() && fileStat.size <= 500_000) {
                ...
                fileCount++;
              } else if (fileStat.size > MAX_FILE_READ_BYTES) {
        ...
        if (stat.isDirectory()) {
          ...
          if (deepPathsSet.has(relPath)) {
            ...
            for (const file of matchingFiles) {
              ...
              try {
                const fileStat = await fsPromises.stat(fileAbsPath);
                if (!fileStat.isDirectory() && fileStat.size <= MAX_FILE_READ_BYTES) {
  ...
  ipcMain.handle("file:write", async (
    _event,
    data: { filePath: string; projectRoot: string; content: string; expectedMtimeMs?: number },
  ) => {
    try {
      const result = await writeTextFile(data);
      return { ok: true, ...result };
    } catch (err) {
      const errMsg = reportError("FILE:WRITE_ERR", err, {
