...
describe("computeToolGroups", () => {
  ...
  it("groups trailing tool calls while the turn is still processing", () => {
    const messages: UIMessage[] = [
      makeAssistant("a0", "Starting", 0),
      makeToolCall("t1", "Bash", 1),
      makeToolCall("t2", "Bash", 2),
      makeToolCall("t3", "Read", 3),
    ];

    const result = computeToolGroups(messages, true, false);
    const group = result.groups.get(1);

    expect(result.groups.size).toBe(1);
    expect(group?.isFinalized).toBe(false);
    expect(group?.tools.map((message) => message.id)).toEqual(["t1", "t2", "t3"]);
    expect([...result.groupedIndices]).toEqual([1, 2, 3]);
  });

  it("does not group a single trailing tool call while processing", () => {
    const messages: UIMessage[] = [
      makeAssistant("a0", "Starting", 0),
      makeToolCall("t1", "Bash", 1),
    ];
    ...
    expect(result.groupedIndices.size).toBe(0);
  });
});
