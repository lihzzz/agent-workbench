...
export function register(): void {
  ipcMain.handle("claude:generate-title", async (_event, {
    ...
    if (engine === "opencode") {
      try {
        const { getOpenCodeSessionModel } = await import("./opencode-sessions");
        const preferredModel = sessionId ? getOpenCodeSessionModel(sessionId) : undefined;
        const { openCodeUtilityPrompt } = await import("../lib/opencode-utility-prompt");
        const raw = await openCodeUtilityPrompt(prompt, cwd || process.cwd(), {
          timeoutMs: 20_000,
          model: preferredModel,
        });
        const title = firstNonEmptyLine(raw) ?? "";
        ...
        return { error: reportError("TITLE_GEN_ERR", err, { engine: "opencode" }) };
      }
    }
    ...
    try {
      ...
      if (engine === "opencode") {
        try {
          ...
          const preferredModel = sessionId ? getOpenCodeSessionModel(sessionId) : undefined;
          const { openCodeUtilityPrompt } = await import("../lib/opencode-utility-prompt");
          const raw = await openCodeUtilityPrompt(prompt, cwd, {
            timeoutMs: 60_000,
            model: preferredModel,
          });
          const message = firstNonEmptyLine(raw) ?? "";
          ...
          return { error: reportError("COMMIT_MSG_GEN_ERR", err, { engine: "opencode" }) };
        }
      }
