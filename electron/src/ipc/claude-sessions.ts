...
import { buildAgentsRecord } from "../lib/subagent-registry";
 ...
 * Claude shares CJK characters across Chinese/Japanese/Korean and, without an
 * explicit language anchor, can drift to a different CJK language mid-session
 * (e.g. answering a Chinese prompt in Japanese). Appended to the default
 * `claude_code` preset so all other behavior is preserved.
 */
const LANGUAGE_SYSTEM_PROMPT =
  ...
  "Never switch to a different language (for example, do not reply in Japanese to a Chinese message) unless the user explicitly asks you to.";

/**
 * Tool-use format anchor. After context compaction the model can regress and
 * emit tool invocations as literal XML text (e.g. `<invoke name="Bash">...`)
 * instead of using the structured tool-use mechanism. This reminder lives in
 * the system prompt, which the SDK always preserves across compaction.
 */
const TOOL_FORMAT_SYSTEM_PROMPT =
  "Always invoke tools using the structured tool-use mechanism. " +
  "Never write tool calls as literal XML text (such as <invoke name=\"...\"> or " +
  "<parameter name=\"...\">) in your reply — that text will not execute and will " +
  "be shown to the user as broken output.";

/** Build the SDK `systemPrompt` option: default claude_code preset + language & tool-format anchors. */
function buildSystemPromptOption() {
  return {
    type: "preset" as const,
    preset: "claude_code" as const,
    append: `${LANGUAGE_SYSTEM_PROMPT}\n\n${TOOL_FORMAT_SYSTEM_PROMPT}`,
  };
}
...
function summarizeSpawnOptions(options: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    agents:
      options.agents && typeof options.agents === "object"
        ? Object.keys(options.agents as Record<string, unknown>)
        : undefined,
    canUseTool: "[callback]",
    stderr: "[callback]",
  };
...
async function restartSession(
  ...
  // Inject globally-defined custom subagents (Claude engine only).
  const customAgents = buildAgentsRecord();
  if (Object.keys(customAgents).length > 0) {
    queryOptions.agents = customAgents;
  }
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ipcMain.handle("claude:start", async (_event, options: StartOptions = {}) => {
    ...
    try {
      ...
      // Inject globally-defined custom subagents (Claude engine only). Read fresh
      // here so the latest definitions apply to every newly started session.
      const customAgents = buildAgentsRecord();
      if (Object.keys(customAgents).length > 0) {
        queryOptions.agents = customAgents;
      }
  ...
  ipcMain.handle("claude:supported-models", async (_event, sessionId: string) => {
    ...
    try {
      ...
      if (Array.isArray(models) && models.length > 0) {
        // setClaudeModelsCache normalizes (and drops the "default" alias);
        // return its result so the picker and cache stay in sync.
        const cached = setClaudeModelsCache(models);
