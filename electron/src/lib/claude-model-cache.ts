...
function normalizeModelInfo(value: unknown): CachedModelInfo | null {
  ...
  // Hide the SDK's "default" alias from the picker. It is kept only as an
  // internal sentinel (see DEFAULT_ENGINE_MODELS / toSdkModel); the renderer
  // resolves a stored "default" to the concrete model it points to.
  if (model.value.trim().toLowerCase() === "default") return null;
