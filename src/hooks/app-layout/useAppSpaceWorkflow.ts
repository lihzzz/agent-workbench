...
export function useAppSpaceWorkflow(input: UseAppSpaceWorkflowInput) {
  ...
  useEffect(() => {
    ...
    if (input.manager.activeSessionId || input.manager.isDraft || draftSpaceIdRef.current) {
      initialSessionRestoreRef.current = true;
      return;
    }
    if (input.projectManager.projects.length === 0) return;
    ...
    initialSessionRestoreRef.current = true;
    void input.manager.switchSession(rememberedSession.id);
  }, [
    input.manager,
    input.manager.activeSessionId,
    input.manager.isDraft,
    input.manager.sessions,
    input.projectManager.projects,
    input.spaceManager.activeSpaceId,
    readLastSessionMap,
  ]);
