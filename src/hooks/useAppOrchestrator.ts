...
export function useAppOrchestrator() {
  ...
  const { subagents, saveSubagent, deleteSubagent } = useSubagents();
  const { templates: promptTemplates, saveTemplate: savePromptTemplate, deleteTemplate: deletePromptTemplate } = usePromptTemplates();
  ...
  const relay = useRelayOrchestrator({
    ...
    activeProjectId: spaceWorkflow.activeProjectId ?? spaceWorkflow.activeSpaceProject?.id ?? null,
    agents,
  ...
  }, [manager.cachedClaudeModels, manager.supportedModels, settings.getModelForEngine, settings.setModelForEngine, settingsEngine]);
  ...
  useEffect(() => {
    ...
    if (session.engine === "opencode") {
      const openCodeAgent = (session.agentId
        ? agents.find((agent) => agent.id === session.agentId)
        : undefined) ?? agents.find((agent) => agent.engine === "opencode");
      if (openCodeAgent && selectedAgent?.id !== openCodeAgent.id) {
        setSelectedAgent(openCodeAgent);
      }
      return;
    }
  ...
  useEffect(() => {
    ...
    const nextPlanMode = manager.activeSession.engine === "opencode"
      ? false
      : getSyncedPlanMode(
