...
async function listCodexModelsWithConfiguredFallback(
  ...
  try {
    const configResult = await rpc.request<ConfigReadResponse>("config/read", {
      includeLayers: false,
      cwd,
    });
    return mergeConfiguredCodexModel(models, configResult.config.model);
  } catch (error) {
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        try {
          models = await listCodexModelsWithConfiguredFallback(rpc, options.cwd);
        ...
        session.threadId = threadResult.thread.id;
        session.model = selectedModel ?? threadResult.model;
        log("codex",` Thread started: ${session.threadId}`);

        void captureEvent("session_created", { engine: "codex", model: session.model });
  ...
  // ─── codex:list-models ───
  ipcMain.handle("codex:list-models", async (_, cwd?: string) => {
    // Try to use any active session's RPC first
    for (const session of codexSessions.values()) {
      if (session.rpc.isAlive) {
        try {
          const models = await listCodexModelsWithConfiguredFallback(session.rpc, cwd ?? session.cwd);
          return { models };
        } catch {
          continue;
        }
    ...
    try {
      ...
      try {
        ...
        rpc.notify("initialized", {});
        const models = await listCodexModelsWithConfiguredFallback(rpc, cwd ?? process.cwd());
        return { models };
      } finally {
        rpc.destroy();
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const requestedModel = normalizeCodexModelOverride(data.model);
        const threadParams = buildCodexThreadResumeParams({
        ...
        session.threadId = threadResult.thread.id;
        session.model = requestedModel ?? threadResult.model;
        log("codex",` Thread resumed: ${session.threadId} model=${session.model ?? "default"}`);

        void captureEvent("session_revived", { engine: "codex", success: true });
        return { sessionId: internalId, threadId: session.threadId, selectedModel: session.model };
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (_, data: { sessionId: string; model: string }) => {
      ...
      const model = normalizeCodexModelOverride(data.model);
