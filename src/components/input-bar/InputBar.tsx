...
import { extractTemplateVars, fillTemplate } from "@/types";
...
import { TemplateVarForm } from "./TemplateVarForm";
...
export const InputBar = memo(function InputBar({
  ...
  // Pending prompt template awaiting {{variable}} fill-in before insertion.
  const [templateForm, setTemplateForm] = useState<{ body: string; vars: string[] } | null>(null);
    ...
    const templateCmds: SlashCommand[] = promptTemplates
      ...
      .map((t) => {
        ...
        return {
          ...
          templateBody: t.body,
          argumentHint: vars.length ? `<${vars.join(", ")}>` : undefined,
        };
      });
  ...
  const selectedModel = useMemo(
    () =>
      modelList.find((m) => m.id === preferredModelId) ??
      (preferredModelId
        ? { id: preferredModelId, label: preferredModelId, description: "" }
        : modelList[0]),
    [modelList, preferredModelId],
  );
  const selectedModelId = selectedModel?.id ?? preferredModelId;

  // Claude effort
  const claudeCurrentModel = useMemo(
    () => supportedModels?.find((m) => m.value === selectedModelId),
    [selectedModelId, supportedModels],
  );
  ...
  // Codex effort
  const codexCurrentModel = useMemo(
    () =>
      codexModelData?.find((m) => m.id === selectedModelId) ??
      codexModelData?.find((m) => m.isDefault) ??
      codexModelData?.[0],
    [codexModelData, selectedModelId],
  );
  const supportedModelCodexEfforts = useMemo(
    () =>
      supportedModels
  ...
  // ── Input history (CLI-style ↑/↓ recall) ──

  const history = useInputHistory(projectPath);
  const [historyOpen, setHistoryOpen] = useState(false);
  // Marks input events caused by our own history recall so handleEditableInput
  // doesn't immediately reset the browse cursor.
  const historyRefillRef = useRef(false);

  const recallFromHistory = useCallback(
    (text: string) => {
      const el = editableRef.current;
      if (!el) return;
      historyRefillRef.current = true;
      setEditableText(el, text);
      hasContentRef.current = hasMeaningfulText(text);
      setHasContent(hasContentRef.current);
    },
    [],
  );

  const selectHistoryEntry = useCallback((entry: string) => {
    recallFromHistory(entry);
    history.reset();
    setHistoryOpen(false);
    requestAnimationFrame(() => editableRef.current?.focus());
  }, [history, recallFromHistory]);
  ...
  const performSend = useCallback(
    ...
    [attachments, projectPath, onSend, clearComposer, grabbedElements, history],
  );
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    // CLI-style input history (only when no picker is open). Up recalls older
    // entries when the caret is on the first line; Down recalls newer entries
    // when on the last line. Otherwise the arrow keys move the caret normally.
    const el = editableRef.current;
    if (el && !isSending) {
      if (e.key === "ArrowUp" && isCaretOnFirstLine(el)) {
        const recalled = history.recallPrev(extractEditableContent(el).text);
        if (recalled !== null) {
          e.preventDefault();
          recallFromHistory(recalled);
          return;
        }
      }
  ...
  const handleEditableInput = useCallback(
    (e: React.FormEvent<HTMLDivElement>) => {
      ...
      // Real keystrokes (not our own history recall) exit the browse cursor so
      // the next Up arrow starts again from the newest entry.
      if (historyRefillRef.current) {
        historyRefillRef.current = false;
      } else {
        history.reset();
      }
      ...
      >
        ...
        {command.showCommands && (
          <CommandPicker
            ...
            onSelect={(cmd) => {
              // Prompt template with {{variables}} → open fill-in form first.
              if (cmd.source === "template" && cmd.templateBody) {
                const vars = extractTemplateVars(cmd.templateBody);
                if (vars.length > 0) {
                  command.setShowCommands(false);
                  setTemplateForm({ body: cmd.templateBody, vars });
                  return;
                }
              }
        ...
        {templateForm && (
          <TemplateVarForm
            vars={templateForm.vars}
            onCancel={() => setTemplateForm(null)}
            onConfirm={(values) => {
              const el = editableRef.current;
              if (el) {
                setEditableText(el, fillTemplate(templateForm.body, values));
                hasContentRef.current = true;
                setHasContent(true);
                el.focus();
              }
        ...
        <div className="mx-4 flex items-center gap-1.5 border-t border-border/[0.08] px-1 pt-2 pb-2.5">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto scrollbar-none">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">Input history</TooltipContent>
            </Tooltip>
            ...
            <EngineControls
              isCodexAgent={isCodexAgent}
              isACPAgent={isACPAgent}
              isOpenCodeAgent={isOpenCodeAgent}
              isProcessing={isProcessing}
            ...
            {/* Current git branch (read-only, click to copy) */}
            <BranchIndicator projectPath={projectPath} />
          </div>

          {/* Right controls */}
          <div className="flex shrink-0 items-center gap-2">
            {!isOpenCodeAgent && contextUsage && contextUsage.contextWindow > 0 && onCompact && (
              <ContextGauge
