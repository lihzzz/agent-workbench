...
interface FolderManager {
  ...
  handleArchiveSession: (sessionId: string, archived: boolean) => Promise<void>;
...
export function useFolderManager({ projects, setSessions }: UseFolderManagerOptions): FolderManager {
  ...
  const handleArchiveSession = useCallback(async (sessionId: string, archived: boolean) => {
    // Mirrors handlePinSession. Archiving also clears the pin so an archived chat
    // never lingers in the pinned section if it's later unarchived in an odd state.
    setSessions((prev) => {
      const session = prev.find((s) => s.id === sessionId);
      if (!session) return prev;
      const patch = archived
        ? { archived: true, pinned: undefined }
        : { archived: undefined };
      window.claude.sessions.updateMeta(session.projectId, sessionId, patch)
        .catch((err) => reportError("handleArchiveSession", err));
      return prev.map((s) =>
        s.id === sessionId
          ? { ...s, archived: archived || undefined, ...(archived ? { pinned: undefined } : {}) }
          : s,
      );
    });
  }, [setSessions]);
