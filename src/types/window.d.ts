...
declare global {
  ...
  interface Window {
    claude: {
      ...
      readFile: (filePath: string) => Promise<{ content?: string; mtimeMs?: number; size?: number; error?: string }>;
      writeFile: (filePath: string, projectRoot: string, content: string, expectedMtimeMs?: number) => Promise<{ ok?: true; mtimeMs?: number; size?: number; error?: string }>;
      ...
      sessions: {
        ...
        export: (args: { format: "markdown" | "json" | "html"; content: string; defaultFileName: string }) => Promise<{
          ok?: true;
          filePath?: string;
          canceled?: true;
          error?: string;
      ...
      codex: {
        ...
        listModels: (cwd?: string) => Promise<{ models: CodexModel[]; error?: string }>;
      ...
      opencode: {
        start: (options: OpenCodeStartOptions) => Promise<OpenCodeStartResult>;
        resume: (options: OpenCodeStartOptions & { resumeSessionId: string }) => Promise<OpenCodeStartResult>;
        send: (sessionId: string, text: string, images?: unknown[]) => Promise<IpcResult>;
        stop: (sessionId: string) => Promise<IpcResult>;
        interrupt: (sessionId: string) => Promise<IpcResult>;
        abortPendingStart: () => Promise<IpcResult>;
        setModel: (sessionId: string, model: string) => Promise<IpcResult>;
        respondPermission: (
          sessionId: string,
          requestId: string,
        ...
        onPermissionRequest: (callback: (data: OpenCodePermissionRequest) => void) => () => void;
        onExit: (callback: (data: OpenCodeExitEvent) => void) => () => void;
      };
      mcp: {
      ...
      subagents: {
        ...
        save: (subagent: CustomSubagent) => Promise<IpcResult>;
        delete: (id: string) => Promise<IpcResult>;
      };
      promptTemplates: {
        list: () => Promise<PromptTemplate[]>;
        save: (template: PromptTemplate) => Promise<IpcResult>;
        delete: (id: string) => Promise<IpcResult>;
      };
      settings: {
