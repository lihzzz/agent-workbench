    ...
    if (item.type === "session") {
      ...
      return (
        <SessionItem
          ...
          onArchiveToggle={() => archiveSession(session.id, !session.archived)}
          folders={folders}
          ...
          onExport={(format) => exportSession(session.id, format)}
        />
      );
    }
      ...
      >
        ...
        >
          <button
            ...
            className="flex w-full min-w-0 items-center gap-1.5 rounded-lg px-2.5 py-2 text-start text-[13px] font-semibold text-sidebar-foreground/90 transition-all group-hover:pe-28 hover:bg-black/5 dark:hover:bg-white/10"
          >
            <ChevronRight
          ...
          <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            >
              ...
              <DropdownMenuItem onClick={onOpenRelay}>
                <Network className="me-2 h-3.5 w-3.5" />
                Open Relay
              </DropdownMenuItem>
              <DropdownMenuCheckboxItem
