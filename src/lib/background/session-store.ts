...
import type { ACPSessionEvent, ACPPermissionEvent, CodexSessionEvent, OpenCodeSessionEvent } from "@/types";
...
import { handleOpenCodeEvent as openCodeHandler } from "./opencode-handler";
...
export interface InternalState extends BackgroundSessionState {
  ...
  /** OpenCode step snapshots can be repeated after SSE reconnects. */
  openCodeProcessedStepIds?: Set<string>;
  openCodeCurrentModel?: string;
}
...
export class BackgroundSessionStore {
  ...
  private getOrCreate(sessionId: string): InternalState {
    ...
    if (!state) {
      state = {
        ...
        activeTask: null,
        openCodeProcessedStepIds: new Set(),
      };
  ...
  /** Handle a native OpenCode event for a background session. */
  handleOpenCodeEvent(event: OpenCodeSessionEvent): void {
    const sessionId = event._sessionId;
    if (!sessionId) return;

    const state = this.getOrCreate(sessionId);
    const result = openCodeHandler(state, event);
    if (result?.processingChanged) {
      this.onProcessingChange?.(sessionId, result.isProcessing ?? false);
    }
  }
