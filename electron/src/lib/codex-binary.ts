 ...
 * 2. App data dir, known install locations, and PATH are scanned and the newest
 *    executable version is selected.
 *
...
import { pickNewestCodexBinary } from "@shared/lib/codex-binary-version";

const STATIC_KNOWN_PATHS: string[] =
...
function getNodeVersionManagerCodexPaths(): string[] {
  ...
  try {
    for (const entry of fs.readdirSync(nvmNodeVersionsDir, { withFileTypes: true })) {
      if (entry.isDirectory()) {
        paths.push(path.join(nvmNodeVersionsDir, entry.name, "bin", "codex"));
      }
    }
  } catch {
    /* nvm is optional */
...
function getKnownCodexPaths(): string[] {
  return [...STATIC_KNOWN_PATHS, ...getNodeVersionManagerCodexPaths()];
}
...
interface CodexBinaryCandidate {
  path: string;
  versionOutput: string | null;
}

function getCodexVersionOutput(binaryPath: string): string | null {
  try {
...
function addExecutableCandidate(
  ...
  if (!normalizedPath || seen.has(normalizedPath) || !isExecutable(normalizedPath)) return;
  seen.add(normalizedPath);
  candidates.push({
    path: normalizedPath,
    versionOutput: getCodexVersionOutput(normalizedPath),
  });
}

function getPathCodexCandidates(): string[] {
  try {
...
function resolveAutoCodexPathSync(): string {
  const candidates: CodexBinaryCandidate[] = [];
  const seen = new Set<string>();

  addExecutableCandidate(candidates, seen, getManagedBinaryPath());
  for (const known of getKnownCodexPaths()) {
    addExecutableCandidate(candidates, seen, known);
  }
  for (const pathCandidate of getPathCodexCandidates()) {
    addExecutableCandidate(candidates, seen, pathCandidate);
  }

  const selected = pickNewestCodexBinary(candidates);
  if (!selected) throw new Error("Codex binary not found");
...
function resolveCodexPathSync(): string {
  ...
  return resolveAutoCodexPathSync();
}
