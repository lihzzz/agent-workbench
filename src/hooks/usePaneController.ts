...
export function usePaneController(
  ...
  return useMemo(() => {
    ...
    const panePlanMode = paneEngine !== "opencode" && (panePermissionMode === "plan"
      || !!session?.planMode
      || (isActiveSessionPane && !session ? ctx.settings.planMode : false));
    const paneSupportedModels = paneEngine === "acp"
      ? []
      : paneEngine === "opencode"
        ? (paneState.opencode.models.length > 0
          ? paneState.opencode.models.map((model) => ({
              value: model.id,
              displayName: model.displayName,
    ...
    const handlePaneSend = async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      if (paneEngine === "opencode") {
        const sent = await paneState.opencode.send(text, images, displayText);
        if (!sent) {
          await ctx.queueSplitPaneSendAfterSwitch?.(sessionId, text, images, displayText);
        }
        return;
