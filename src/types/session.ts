...
export type RelayStageRole = "plan" | "review-plan" | "fix-plan" | "implement" | "code-review";
...
export interface UIMessage {
  ...
  isQueued?: boolean;
  /**
   * When true, this tool_call was reconstructed from tool-call markup the model
   * emitted as plain text (a long-context/post-compaction failure mode). It was
   * never actually executed -- rendered with a distinct "not run" visual state.
   */
  isLeakedToolCall?: boolean;
}
...
export interface SessionBase {
  ...
  /** Native OpenCode session ID used to reconnect after app restart. */
  opencodeSessionId?: string;
  /** Relay workflow group and lane metadata. */
  workflowGroupId?: string;
  workflowGroupName?: string;
  stageRole?: RelayStageRole;
  stageIndex?: number;
  ...
  pinned?: boolean;
  /** Whether this chat is archived (hidden from the sidebar, shown in Settings → Archived). */
  archived?: boolean;
...
export interface PersistedSession extends SessionBase {
  ...
  /** Cumulative per-model token/cost usage (Claude only). Absent for old/ACP/Codex sessions. */
