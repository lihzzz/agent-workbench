...
export interface ToolGroupInfo {
  ...
  /** Set of all message indices that belong to any rendered group (for skip logic). */
 ...
 * Active trailing groups are also included while a turn is processing so long
 * command runs do not render as a tall stack of individual tool rows.
 */
export function computeToolGroups(
  ...
  const finalizeGroup = (isFinalized: boolean) => {
  ...
  // Handle trailing group (tools at end of messages). While processing, keep it
  // as an active group; once the turn is idle, the same group becomes finalized.
  if (currentTools.length >= 2) {
    finalizeGroup(!isProcessing);
  }
