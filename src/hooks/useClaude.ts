...
import { parseLeakedToolCalls, stripStreamingLeak } from "../lib/engine/leaked-tool-parse";
import {
  getParentId,
  extractTextContent,
...
export function useClaude({ sessionId, initialMessages, initialMeta, initialPermission }: UseClaudeOptions) {
  ...
  const flushStreamingToState = useCallback(() => {
    // Hide any leaked tool-call markup from the visible text mid-stream so
    // half-written XML never flashes as raw text. Authoritative parsing (and
    // synthetic tool cards) happens once in the `assistant` snapshot handler.
    const allText = stripStreamingLeak(buffer.current.getAllText());
    ...
    (event: ClaudeEvent & { _sessionId?: string }) => {
      ...
      switch (event.type) {
        ...
        case "assistant": {
          ...
          const rawTextContent = extractTextContent(event.message.content);
          const thinkingContent = extractThinkingContent(event.message.content);

          // The model sometimes (esp. after compaction) emits tool-call markup
          // as plain text instead of a structured tool_use block. Strip it from
          // the visible text and reconstruct synthetic, un-executed tool cards.
          const { cleanedText: textContent, calls: leakedCalls } =
            parseLeakedToolCalls(rawTextContent);
          ...
          // Append a synthetic tool_call card for each leaked invocation so the
          // user sees what the model intended — clearly marked as never executed.
          if (leakedCalls.length > 0) {
            setMessages((prev) => {
              const additions = leakedCalls
                .map((call, idx) => ({
                  id: `leaked-${event.uuid}-${idx}`,
                  role: "tool_call" as const,
                  content: "",
                  toolName: call.name,
                  toolInput: normalizeTodoToolInput(call.name, call.input),
                  timestamp: Date.now(),
        ...
        case "result": {
          ...
          if (resultEvent.modelUsage) {
            ...
            // Accumulate per-model token/cost usage for the cost dashboard.
            setModelUsage((prev) => mergeModelUsage(prev, resultEvent.modelUsage!));
          }
