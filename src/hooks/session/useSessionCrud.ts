...
interface UseSessionCrudParams {
  ...
  prefetchOpenCodeModels: (preferredModel?: string) => Promise<void>;
...
export function useSessionCrud({
  ...
  const createSession = useCallback(
    async (projectId: string, options?: StartOptions) => {
      ...
      if ((startOptionsRef.current.engine ?? "claude") === "opencode") {
        void window.claude.opencode.abortPendingStart();
      }
      ...
      if (draftEngine === "claude") {
        ...
        setDraftMcpStatuses([]);
        prefetchOpenCodeModels(options?.model);
      }
    },
    [saveCurrentSession, seedBackgroundStore, eagerStartSession, eagerStartAcpSession, abandonEagerSession, abandonDraftAcpSession, prefetchCodexModels, prefetchOpenCodeModels, probeMcpServers],
  );
      ...
      if (bgState) {
        startTransition(() => {
          ...
          setInitialMeta({
            ...
            // Preserve the live engine permission mode (incl. "plan") so plan state
            // survives switch-away/switch-back instead of being silently reset.
            sessionInfo: bgState.sessionInfo,
            totalCost: bgState.totalCost,
            contextUsage: bgState.contextUsage,
            modelUsage: bgState.modelUsage,
    ...
    async (id: string) => {
      ...
      if (liveSessionIdsRef.current.has(id)) {
        if (session.engine === "codex") {
          ...
          suppressNextSessionCompletion(id);
          await window.claude.opencode.stop(id);
        } else {
          suppressNextSessionCompletion(id);
  ...
  const setDraftAgent = useCallback((draftEngine: string, agentId: string, _cachedConfigOptions?: ACPConfigOption[], model?: string) => {
    ...
    if (prevEngine === "opencode" && draftEngine !== "opencode") {
      void window.claude.opencode.abortPendingStart();
    }
    ...
    if (draftEngine === "codex") {
      ...
      prefetchOpenCodeModels(normalizedModel || undefined);
  ...
  }, [prefetchCodexModels, prefetchOpenCodeModels, abandonEagerSession, abandonDraftAcpSession, eagerStartAcpSession, probeMcpServers]);
