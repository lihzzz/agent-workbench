...
export function AppLayout() {
  ...
  const {
    sidebar, projectManager, spaceManager, manager, settings, resolvedTheme, spaceTerminals, activeSpaceTerminals, splitView, relay,
  } = managers;
  const {
    agents, selectedAgent, saveAgent, deleteAgent, handleAgentChange, lockedEngine, lockedAgentId,
    subagents, saveSubagent, deleteSubagent,
    promptTemplates, savePromptTemplate, deletePromptTemplate,
  } = agentState;
  const {
  ...
  const handleOpenRelay = useCallback(
    async (projectId: string) => {
      const project = projectManager.projects.find((item) => item.id === projectId);
      if (project) {
        setJiraBoardProjectForSpace(project.spaceId || "default", null);
      ...
      splitView.dismissSplitView();
      await relay.openRelay(projectId);
    },
    [projectManager.projects, relay.openRelay, setJiraBoardProjectForSpace, splitView.dismissSplitView],
  );
  ...
  const createSplitPaneDraftSession = useCallback(
    async (replacedSessionId: string, projectId: string, agent: InstalledAgent | null) => {
      ...
      await manager.createSession(projectId, {
        model: wantedModel,
        permissionMode: wantedEngine === "opencode" ? undefined : settings.permissionMode,
        planMode: wantedEngine === "opencode" ? false : settings.planMode,
  ...
  }), [agents, selectedAgent, saveAgent, deleteAgent, handleAgentChange, lockedEngine, lockedAgentId, subagents, saveSubagent, deleteSubagent, promptTemplates, savePromptTemplate, deletePromptTemplate]);
    ...
    >
      ...
      <AppSidebar
        ...
        sessionActions={{
          ...
          onArchiveSession: o.handleArchiveSession,
            ...
            >
              <RightPanel
                ...
                content={{
                  ...
                  activeTodos,
                  isProcessing: manager.isProcessing,
                  onCloseTodos: () => handleToggleTool("tasks"),
                  bgAgents,
