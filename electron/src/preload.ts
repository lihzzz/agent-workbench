...
contextBridge.exposeInMainWorld("claude", {
  ...
  writeFile: (filePath: string, projectRoot: string, content: string, expectedMtimeMs?: number) =>
    ipcRenderer.invoke("file:write", { filePath, projectRoot, content, expectedMtimeMs }),
  ...
  sessions: {
    ...
    export: (args: { format: "markdown" | "json" | "html"; content: string; defaultFileName: string }) =>
      ipcRenderer.invoke("sessions:export", args),
  ...
  opencode: {
    start: (options: { cwd: string; model?: string; resumeSessionId?: string }) =>
      ipcRenderer.invoke("opencode:start", options),
    resume: (options: { cwd: string; model?: string; resumeSessionId: string }) =>
      ipcRenderer.invoke("opencode:resume", options),
    send: (sessionId: string, text: string, images?: unknown[]) =>
      ipcRenderer.invoke("opencode:send", { sessionId, text, images }),
    stop: (sessionId: string) => ipcRenderer.invoke("opencode:stop", sessionId),
    interrupt: (sessionId: string) => ipcRenderer.invoke("opencode:interrupt", sessionId),
    abortPendingStart: () => ipcRenderer.invoke("opencode:abort-pending-start"),
    setModel: (sessionId: string, model: string) =>
      ipcRenderer.invoke("opencode:set-model", { sessionId, model }),
    respondPermission: (sessionId: string, requestId: string, reply: "once" | "always" | "reject") =>
      ipcRenderer.invoke("opencode:respond-permission", { sessionId, requestId, reply }),
    listModels: (cwd: string) => ipcRenderer.invoke("opencode:list-models", cwd),
    version: () => ipcRenderer.invoke("opencode:version"),
    binaryStatus: () => ipcRenderer.invoke("opencode:binary-status"),
    onEvent: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
    ...
    onPermissionRequest: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
      ipcRenderer.on("opencode:permission_request", listener);
      return () => ipcRenderer.removeListener("opencode:permission_request", listener);
    },
    onExit: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
      ipcRenderer.on("opencode:exit", listener);
      return () => ipcRenderer.removeListener("opencode:exit", listener);
    },
  },
  mcp: {
  ...
  subagents: {
    ...
    save: (subagent: unknown) => ipcRenderer.invoke("subagents:save", subagent),
    delete: (id: string) => ipcRenderer.invoke("subagents:delete", id),
  },
  promptTemplates: {
    list: () => ipcRenderer.invoke("prompt-templates:list"),
    save: (template: unknown) => ipcRenderer.invoke("prompt-templates:save", template),
    delete: (id: string) => ipcRenderer.invoke("prompt-templates:delete", id),
  },
  settings: {
