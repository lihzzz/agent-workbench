...
export interface EngineBaseState {
  ...
  setModelUsage: Dispatch<SetStateAction<SessionModelUsage>>;
...
export function useEngineBase({
  ...
  const [modelUsage, setModelUsage] = useState<SessionModelUsage>(initialMeta?.modelUsage ?? {});
  ...
  useEffect(() => {
    ...
    if (initialMeta) {
      ...
      setModelUsage(initialMeta.modelUsage ?? {});
    } else {
  ...
  // Session payloads can arrive after the session id during disk revival. Hydrate
  // only while the engine is still empty so live streaming state always wins.
  useEffect(() => {
    if (!sessionId || !initialMessages || initialMessages.length === 0) return;
    ...
    setPendingPermission(initialPermission ?? null);
  }, [initialMessages, initialMeta, initialPermission, sessionId]);
