...
export function useMessageQueue({ refs, setters, engines, activeSessionId }: UseMessageQueueParams) {
  const { claude, acp, codex, opencode, engine } = engines;
  ...
  }, [activeSessionIdRef, acp.setMessages, backgroundStoreRef, claude.setMessages, codex.setMessages, opencode.setMessages]);
  ...
  }, [activeSessionIdRef, acp.setIsProcessing, backgroundStoreRef, claude.setIsProcessing, codex.setIsProcessing, opencode.setIsProcessing]);
  ...
  const drainQueuedMessageForSession = useCallback(async (sessionId: string) => {
    ...
    try {
      if (sessionEngine === "opencode") {
        ...
        const result = await window.claude.opencode.send(sessionId, next.text, next.images);
  ...
  useEffect(() => {
    ...
    } else if (sessionEngine === "opencode") {
      void window.claude.opencode.interrupt(activeId);
    } else {
