...
export function useSessionSettings({
  ...
  const { claude, opencode, engine } = engines;
  const {
    setSessions,
    setStartOptions,
  ...
  const setActiveModel = useCallback((model: string) => {
    ...
    const isLiveCodexSession = (session.engine ?? "claude") === "codex"
      && liveSessionIdsRef.current.has(id);
    const isLiveOpenCodeSession = session.engine === "opencode"
      && liveSessionIdsRef.current.has(id);
    ...
    if (isLiveOpenCodeSession) {
      opencode.setModel(model).then(() => persistModel()).catch((error) => {
    ...
    persistModel();
  }, [abandonEagerSession, claude.setModel, eagerStartSession, getClaudeSdkModel, opencode.setModel, persistSessionPatch, resetCodexEffortToModelDefault]);
  ...
  const setSessionModel = useCallback(async (sessionId: string, model: string) => {
    ...
    if (session.engine === "opencode" && liveSessionIdsRef.current.has(sessionId)) {
