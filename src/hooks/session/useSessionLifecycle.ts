...
interface UseSessionLifecycleParams {
  ...
  reviveOpenCodeSession: (text: string, images?: ImageAttachment[], displayText?: string) => Promise<void>;
  // From message queue
    ...
    async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      if (activeId === DRAFT_ID) {
        ...
        if (draftEngine === "opencode") {
          ...
          opencode.setIsProcessing(true);
          const sendResult = await window.claude.opencode.send(sessionId, text, images);
          if (sendResult.error) {
            opencode.setMessages((previous) => [
              ...previous,
              createSystemMessage(`Unable to send message: ${sendResult.error}`, true),
      ...
      if (activeSessionEngine === "opencode") {
        if (refs.liveSessionIdsRef.current.has(activeId)) {
          await opencode.send(text, images, displayText);
          return;
        }
        await reviveOpenCodeSession(text, images, displayText);
        return;
      }
