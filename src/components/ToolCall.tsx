...
export const ToolCall = memo(function ToolCall({
  ...
  prev.message.isLeakedToolCall === next.message.isLeakedToolCall &&
...
const RegularTool = memo(function RegularTool({
  ...
  const isLeaked = !!message.isLeakedToolCall;
  // A leaked tool call was never executed, so it is neither "running" (awaiting
  // a result) nor a hard error — give it its own state below.
  const isRunning = !hasResult && !isLeaked;
  ...
  const triggerContent = (
    <div className="relative flex min-w-0 flex-1 items-center gap-[6.4px]">
      {showToolIcons && ((isError || isLeaked) ? (
        <ToolGlyph Icon={AlertCircle} className={isLeaked ? "text-amber-400/70" : "text-red-400/70"} />
      ) : (
      ...
      {isRunning ? (
        ...
        <span className="shrink-0 whitespace-nowrap font-medium text-amber-400/70">
          {`Suggested ${message.toolName ?? "tool"} (not run)`}
        </span>
      ) : (
