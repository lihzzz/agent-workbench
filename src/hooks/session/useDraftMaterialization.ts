...
export function useDraftMaterialization({
  ...
  const prefetchCodexModels = useCallback(async (preferredModel?: string) => {
    ...
    try {
      ...
      const project = draftProjectIdRef.current
        ? refs.projectsRef.current.find((entry) => entry.id === draftProjectIdRef.current)
        : undefined;
      const result = await window.claude.codex.listModels(project ? getProjectCwd(project) : undefined);
      if (result.error) {
  ...
  }, [applyCodexModelDefaultEffort, codex.setCodexModels, draftProjectIdRef, getProjectCwd, refs.projectsRef, setCodexModelsLoadingMessage, setCodexRawModels, setStartOptions]);

  const prefetchOpenCodeModels = useCallback(async (preferredModel?: string) => {
    const activeProjectId = activeSessionIdRef.current && activeSessionIdRef.current !== DRAFT_ID
      ? refs.sessionsRef.current.find((session) => session.id === activeSessionIdRef.current)?.projectId
      : undefined;
    const projectId = draftProjectIdRef.current ?? activeProjectId;
  ...
  const materializeDraft = useCallback(
    async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      const workflowMeta = options.workflowGroupId
        ? {
            workflowGroupId: options.workflowGroupId,
            workflowGroupName: options.workflowGroupName,
      ...
      if (draftEngine === "acp" && options.agentId) {
        ...
        const result = await window.claude.opencode.start({
          cwd: getProjectCwd(project),
          model: options.model,
        });
        if (result.error || !result.sessionId || !result.opencodeSessionId) {
          const errorMessage = result.error || "Failed to start OpenCode session";
          const failedId = `failed-opencode-${Date.now()}`;
          const errorMessages: UIMessage[] = [
            createUserMessage(text, images, displayText),
          ...
          setSessions((previous) => previous.map((session) =>
            session.id === DRAFT_ID
              ? { ...session, id: failedId, titleGenerating: false }
              : session,
          ));
          setInitialMessages(errorMessages);
          setInitialMeta({
        ...
        sessionModel = result.selectedModel ?? options.model;
