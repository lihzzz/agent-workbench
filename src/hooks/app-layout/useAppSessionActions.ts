...
export function useAppSessionActions(input: UseAppSessionActionsInput) {
  const getClaudeEffortForModel = useCallback((model: string | undefined): ClaudeEffort | undefined => {
    ...
    const catalog = input.manager.cachedClaudeModels.length > 0
      ? input.manager.cachedClaudeModels
      : input.manager.supportedModels;
    const meta = catalog.find((entry) => entry.value === model);
    ...
    return levels[0];
  }, [input.manager.cachedClaudeModels, input.manager.supportedModels, input.settings.claudeEffort]);
  ...
  const handleSelectSession = useCallback((sessionId: string) => {
    input.setShowSettings(false);
    // Don't force plan mode off here — the plan toggle is driven by the target
    // session's persisted planMode via useAppOrchestrator's sync effect. Forcing
    // false would silently disable (and persist) plan mode on switch-back.
  ...
  const handleNavigateToMessage = useCallback((sessionId: string, setScrollToMessageId: (messageId: string) => void, messageId: string) => {
    // Plan toggle is synced from the target session by useAppOrchestrator — don't
    // force it off here (that would silently disable plan mode on the target).
    input.manager.switchSession(sessionId);
